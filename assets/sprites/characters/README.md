...
> **`buffer_pure_walk.png` 의 이름은 역사적 흔적이다**(#276). 이 아트를 쓰던 `buffer_pure` 는
> 설아가 순혈 버퍼 슬롯을 이어받으면서 삭제됐고, 아트는 새 겸직 플레이스홀더 `buffer_ranged` 가
> 물려받았다. 파일 이름을 바꾸지 않은 이유: 재임포트와 UID 갱신이 따라붙는 변경이라, 이름이
