...
| 영문명 | 한국명 | 설명 |
|--------|---------|------|
| protein | 프로틴 | 삼각근 Lv.의 성장 재료. 잔액이 `PlayerProfile.required_protein(현재 Lv.)` 이상이 되면 **자동으로 그만큼 차감되며** Lv.이 오른다 (#204) |

프로틴은 다른 재화와 달리 **플레이어가 쓰는 것이 아니라 시스템이 자동으로 소모한다.**
`CurrencySystem.add_currency("protein", n)` 이면 경로가 무엇이든(스테이지 클리어·상점·우편)
`PlayerProfile` 이 `currency_added` 신호를 듣고 정산한다. 지급하는 쪽은 아무것도 더 하지 않는다.
