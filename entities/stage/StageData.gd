...
## **개발자용.** 왜 이 구성인가. 이슈 번호·`[임시값]` 표시가 섞여 있다.
## 화면에 그리지 않는다 — 아래 summary 가 그 몫이다(#360).
@export_multiline var description: String = ""

## **플레이어용.** 이 스테이지에서 무엇을 하는가. 출격 화면이 보여 준다 (#360).
##
## description 을 그대로 그렸더니 플레이어가 "튜토리얼 스테이지(#345) … #340, #342 —
## 스탯이 같고 외형만 다르다 … #208 로 탐지 범위가 600 이 되어" 를 읽게 됐다.
## 그렇다고 description 을 다듬으면 설계 근거가 사라진다. 스킬(#353)과 같은
## 방식으로 필드를 나눈다.
##
## 규약(data/skills/README.md 와 같다):
##   - 이슈 번호·마크다운·`[임시값]`·필드명을 쓰지 않는다.
##   - **수치를 글로 다시 적지 않는다.** 챕터·타입·웨이브 수는 화면이 필드에서
##     읽어 따로 그린다. 여기에 또 적으면 데이터를 고칠 때 둘이 갈라진다.
