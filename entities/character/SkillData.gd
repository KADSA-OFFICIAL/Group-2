...
## **설계 근거**. 왜 이렇게 정했는지를 적는 개발자용 문서다.
## 이슈 번호·필드명·`[임시값]` 표시가 들어간다 — 화면에 그대로 내보내지 않는다.
@export_multiline var description: String = ""

## **플레이어용 한 줄 설명**(#353). 캐릭터 화면의 스킬 카드가 이것을 보여 준다.
##
## description 과 나눠 둔 이유: 읽는 사람이 다르다. description 은 "왜 이 수치인가"를
## 남기는 자리라 이슈 번호와 [임시값] 표시가 섞여 있어 플레이어에게 보일 수 없다.
## 그렇다고 description 을 다듬으면 설계 근거가 사라진다.
##
## 규약:
##   - 이슈 번호·마크다운·`[임시값]`·필드명을 쓰지 않는다.
##   - **수치를 글로 다시 적지 않는다.** 쿨타임·반경·위력은 카드가 필드에서 읽어
##     따로 그린다. 여기에 또 적으면 데이터를 고칠 때 둘이 갈라진다.
##   - 무엇을 하는지와, 수치로 드러나지 않는 성질(관통·대상 선정·조건)만 적는다.
...
# ===== 대상 최대 체력 비례 피해 (Target max-HP damage) =====

## 맞은 **대상의 최대 체력 x 이 비율**만큼 때린다. 0 이면 쓰지 않는다.
##
## 위 current_hp_damage_percent 와 반대 축이다(태희 E 와 하랑 E 의 관계와 같다).
## 그쪽은 **현재** 체력 비례라 깎일수록 약해지고 마무리를 못 하지만, 이쪽은 **최대** 체력
## 비례라 체력이 얼마 남았든 같은 값이 들어간다 — **맞으면 죽을 수 있다.**
##
## attack_override_max_hp_percent 와 나눠 둔 이유: 그쪽은 "평타가 바뀐 창 동안의 평타 피해"라
## 그 창에 묶여 있다. 이쪽은 스킬 자신이 한 번 내는 피해다. 한 필드로 뭉치면 "언제 나가는
## 피해인가"가 데이터에서 사라진다.
##
## 이 무게를 쓰려면 **피할 수 있어야 한다.** 지금 이 필드를 쓰는 것은 보스의 특별 스킬 둘이고,
## 각각 준비자세 2초와 착지 예고로 회피 창을 준다(#376).
##
## 방어력은 기존 규약대로 대상의 take_damage() -> PlayerStats.apply_defense() 가 적용한다.
@export var target_max_hp_damage_percent: float = 0.0

## 이 스킬이 **몇 번 반복되는가**. 0 이나 1 이면 한 번이다.
##
## 보스의 3연 도약(#376)처럼 같은 판정이 연달아 나가는 스킬이 쓴다. 반복마다 대상 위치를
## 다시 읽으므로, 예고를 보고 움직이면 뒤 타는 피할 수 있다.
