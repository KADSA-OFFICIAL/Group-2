...
#   zoom 0.85 / 뷰포트 720 -> 보이는 높이 847, 반높이 424
#   offset.y = -140 이면 발이 위에서 564px(67%), 머리가 445px(53%) 지점에 온다.
#
# ===== 왜 zoom 0.85 인가 (#392) =====
#
...
#
#   zoom 0.85 / 뷰포트 1280x720 -> 보이는 세계 1506x847, 반폭 753
#   753 > 600(탐지) 이라 적이 다가오기 시작하는 순간이 화면 안에 들어온다.
#   #386 의 수비 반경 320(지름 640)도 여유 있게 담긴다.
#   캐릭터 키 119px 는 화면 높이의 14%가 된다.
#
# **더 물릴 수 없는 이유**: 초원 맵(stage/grassland)이 64x40 타일 x 24px = 1536x960 이다.
# zoom 0.8 이면 보이는 폭이 1600 이 되어 **맵 폭보다 화면이 넓어지므로** 어디를 봐도 맵
# 바깥의 빈 공간이 드러난다. 0.85(1506)는 그 직전 값이다.
# 더 멀리 가려면 맵을 넓히거나 카메라 경계 제한(limit_*)을 먼저 넣어야 한다.
#
# offset.y 를 -80 에서 -140 으로 함께 올린 이유: 발 위치 비율(67%)을 그대로 두기 위해서다.
# zoom 만 바꾸면 반높이가 240 -> 424 로 커져 발이 화면 중앙 쪽으로 올라간다.
