...
## 사거리 제한 해제 — `EMPOWER` 의 네 번째 payload (#336)

`grants_unlimited_attack_range` 가 켜져 있으면 `Player.get_attack_range()` 가 무제한을
돌려준다 — 살아 있는 적이 있으면 거리와 무관하게 평타가 나간다.

**"사거리 +N%"(STAT_MOD)로 만들 수 없다.** 사거리는 `PlayerStats` 의 스텟이 아니다.
그 값의 출처는 `CharacterData.basic_attack_range` 이거나 씬의 `AttackArea2D` 이고 버프 채널이
없다. 무엇보다 스펙이 "**제한 해제**"라 배율로는 표현되지 않는다 — 무한대를 곱으로 만들 수 없다.
숫자가 아니라 분기이므로 `EMPOWER` 다(흡혈·처형·무적과 같은 판단).

대상 선정은 바뀌지 않는다: 여전히 **최근접 적**이다(`Player._find_attack_target`).
이 효과가 푸는 것은 그 선정의 **거리 제한**뿐이다.

## 같은 CC 라도 지속시간이 다르면 효과를 따로 만든다 (#336)

`stun`(2.0초)과 `shock_stun`(0.3초)은 막는 행동이 완전히 같다. 그래도 하나로 합치지 않았다.

지속시간은 **효과 자신의 성질**이라 `StatusEffectData` 가 소유하는데
(`SkillData.cooldown` 이 스킬의 성질인 것과 같다), 하나로 합쳐 부르는 쪽이 시간을 넘기게 하면
그 규약이 깨진다. 실제로도 출처가 다르다 — `stun` 은 탱커 표식이 임계치에서 터뜨리는 한 방이고,
`shock_stun` 은 `ranged_tank` 의 평타마다 걸리는 짧은 끊김이다. 합치면 한쪽을 조정할 때
다른 쪽이 조용히 따라 바뀐다.
