...
| `pterosaur_queen` | 원거리 보스. 사거리 320 에서 쏘고 강화탄으로 기절. 패턴 3종(후퇴·관통 직선·속박 지대). 대본 화자를 겸한다 (#375, #391) |

## 보스 스크립트는 BossEnemyBase 위에 얹는다 (#391)

체력 임계선·안무 뼈대·예고는 [`BossEnemyBase`](../../entities/enemies/BossEnemyBase.gd) 가
갖는다. 보스 스크립트는 그것을 상속받아 **자기 안무만** 채운다
(`_start_skill()` 과 `_tick_act()`).

| 보스 | 공략 축 | 안무 |
|---|---|---|
| `MammothBoss` | 붙어서 버틴다 | 준비-돌진 / 3연 도약 — **따라붙는다** |
| `PterosaurQueen` | 거리를 좁힌다 | 후퇴 / 관통 직선 / 속박 지대 — **떼어놓는다** |

두 보스가 같은 뼈대 위에서 방향만 반대다. 임계선 규칙을 보스마다 다시 쓰면 한쪽만
고쳐지는 순간이 오므로 여기서 한 벌만 둔다(CLAUDE.md 단일 출처).

**어느 안무인지는 스킬 데이터가 정한다.** 스크립트가 `phase_skills` 인덱스로 분기하면
`.tres` 순서를 바꿀 때 조용히 다른 안무가 나간다 — 필드의 유무(`aura_radius`,
`beam_length`, `repeat_count`)로 고른다.
