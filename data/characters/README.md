...
**플레이스홀더는 더 이상 없다.** `ranged_tank` 도 #336 에서 평타와 고유 스킬을 받았다 — 다만 **이름·개성은 아직 미정**이라 `character_id` 와 `display_name` 이 역할 표기 그대로다(임의로 짓지 않는다).
...
- **고유 스킬(`skills`)은 로스터 6명 전원에게 저작되어 있다**(`mina` #259 · `taehee` #263 · `seola` #276 · `harang` #328 · `gangji` #334 · `ranged_tank` #336).
