...
**`shipduck`·`mina`·`taehee`·`seola` 를 제외한 2명(`ranged_tank`, `buffer_pure`)은 역할 구성만 채운 플레이스홀더다.**
...
- **고유 스킬(`skills`)은 `mina`(#259)·`taehee`(#263) 둘만 저작되어 있고 나머지 4명은 비어 있다.**
  - 설계 제약: 고유 스킬 중 **일부는 힐 또는 보호막을 제공해야 한다.** 버퍼 3단계 시너지가 이를 전제로 하기 때문이다(§3, §8.1). 지금은 미나가 이 제약을 충족한다.
