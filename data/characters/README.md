...
**플레이스홀더는 더 이상 없다.** 마지막까지 남아 있던 `ranged_tank` 는 #336 에서 평타와 고유 스킬을 받고 #338 에서 **아린**(`arin`)이라는 이름을 받았다 — 이로써 **로스터 6명 전원이 이름과 전투 내용을 갖는다.**

다만 **개성·서사는 여섯 명 모두 미정**이다. 이름과 전투는 확정이고 그 위에 얹힐 인물 설정이 남아 있다 — 임의로 짓지 않는다.
...
> 플레이스홀더 `ranged_tank`(지금의 아린)가 이어받았고, 태희가 들어간 순혈 자리에 있던 `ranged_pure` 는
...
- **고유 스킬(`skills`)은 로스터 6명 전원에게 저작되어 있다**(`mina` #259 · `taehee` #263 · `seola` #276 · `harang` #328 · `gangji` #334 · `arin` #336).
