...
|---|---|---|---|
| `shipduck.tres` | `shipduck` | 버퍼 + 원거리딜러 (겸직) | 파랑 |
...
> 그 자리에 있던 shipduck 은 **겸직(버퍼+원거리)** 로 맞바꿨다. 하랑만 옮기면 겸직 조합이 겹쳐
...
> 그대로 들고 갔고 shipduck 도 그대로다. 하랑이 탱커 성향인데 체력 900 이 로스터 최저인 것은
> 밸런싱에서 함께 볼 문제로 남겨 두었다(#259·#263 은 슬롯을 따라 스텟을 맞바꿨는데, 이번에는
> 두 인물 모두 고유 스킬·개성이 붙어 있어 스텟을 교환하면 그쪽이 어긋난다).
>
> 미나와 shipduck 의 역할은 #259 에서 맞바꿨다 — 미나가 순혈 탱커, shipduck 이 겸직(탱커+버퍼)이었다(#328 에서 다시 옮겼다).
...
- **고유 스킬(`skills`)은 `mina`(#259)·`taehee`(#263)·`seola`(#276)·`harang`(#328) 넷이 저작되어 있고 나머지 2명(`shipduck`, `ranged_tank`)은 비어 있다.**
