...
@onready var diamond_label = Label.new()
...
func _ready():
	...
	# Setup Diamond display
	diamond_label.text = "Diamond: 0"
	diamond_label.add_theme_font_size_override("font_size", 24)
	add_child(diamond_label)
	diamond_label.anchor_left = 0.0
	diamond_label.anchor_top = 0.0
	diamond_label.offset_left = 20
	diamond_label.offset_top = 60
...
func _update_display():
	if CurrencySystem:
		...
		diamond_label.text = "Diamond: %d" % diamond
