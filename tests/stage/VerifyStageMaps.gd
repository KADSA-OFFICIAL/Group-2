...
#
# 스폰 거리를 여기서 검사하는 이유: 적이 detection_range 밖에 있으면 플레이어가
# 걸어가기 전까지 아무 일도 일어나지 않아 "적이 안 나온다"처럼 보인다.
#
# 파티 시작 자리는 **Stage 에서 가져온다**(#448). 예전에는 좌표를 여기 베껴 적었는데,
# 스폰이 바뀌면 이 파일이 조용히 낡았다 — 실제로 스폰이 울타리 충돌 안이라는 것도
# 그래서 드러나지 않았다.
const PARTY_START := Stage.PARTY_SPAWN_ORIGIN + Stage.PARTY_SPAWN_STEP  # 3인 파티의 가운데 자리
