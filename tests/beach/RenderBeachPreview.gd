...
# 해변 맵 미리보기 PNG 를 뽑는다 (#422).
# tests/grassland/RenderGrasslandPreview.gd 와 같은 관례다.
#
...
#             여기서 물가가 보이지 않으면 "해변"이라고 적힌 맵이 모래밭으로만 보인다.
#   - overview: 맵 전체. 물과 모래의 비율, 물가의 출렁임, 프롭 분포를 한눈에 본다.
...
func _render(label: String, camera_position: Vector2, zoom: float) -> bool:
	...
	world.add_child(BEACH_MAP.instantiate())
	...
	var output_path := "user://beach-map-%s.png" % label
	var error := viewport.get_texture().get_image().save_png(output_path)
	viewport.queue_free()
	if error != OK:
		push_error("Could not save beach preview (%s): %s" % [label, error_string(error)])
		return false
