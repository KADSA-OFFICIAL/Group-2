...
## 몸이 112 폭이 되었으므로 간격을 넓혀 5체가 겹치지 않게 한다.
## **1280 폭에서 맨 오른쪽 적이 화면을 넘지 않는 것이 상한을 정한다** — 지금
## 5번 적의 몸 우단이 1260, 화면 끝까지 20px 남는다.
...
##
## 값의 근거: 전투 화면이 원소 문양 라벨을 발밑 기준 -185 에 놓고 그 글자가 32px 라
## 발밑 -185 ~ -153 을 쓴다. 클러스터 밑변을 그 위로 넘기려면 정수리(-140)에서
## 54 이상 띄워야 한다. **몸 크기를 바꿀 때마다 이 셋(몸 높이 · 문양 위치 · 이 값)이
## 함께 움직여야 한다** — 하나만 두면 글자나 바가 몸 안으로 들어간다.
const CLUSTER_GAP := 54.0
...
const AIM_FRAME := Vector2(106.0, 128.0)
...
## 몸이 101 폭이 되었으므로 간격을 넓힌다. 간격이 몸 폭과 같으면 정확히 맞닿는다.
## 1번 아군의 몸 우단(595)이 1번 적의 몸 좌단(644)보다 왼쪽이어야 한다.
