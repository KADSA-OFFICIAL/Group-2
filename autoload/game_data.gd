...
# ── 재화 ──
# 지갑 재화(gold/gems/faction_token/sweep_tickets)는 CurrencySystem 이 단일 원천(source of truth).
# 아래 프로퍼티는 CurrencySystem 으로 위임하는 프록시일 뿐, 값을 자체 보관하지 않는다.
# UI 는 종전처럼 GameData.gold 등을 읽고 GameData.currency_changed 를 구독하면 된다.
var stamina: int = 93
var stamina_max: int = 206

# 지갑 재화 기본값 (세이브가 없을 때 최초 1회 시드)
const _WALLET_DEFAULTS := {
	"gold": 130_605_248,
	"gems": 51_118,
	"faction_token": 1_250,
	"sweep_tickets": 40,
...
var gold: int:
	get: return CurrencySystem.get_balance("gold")
	set(v): CurrencySystem.set_currency("gold", maxi(0, v))
var gems: int:
	get: return CurrencySystem.get_balance("gems")
	set(v): CurrencySystem.set_currency("gems", maxi(0, v))
var faction_token: int:
	get: return CurrencySystem.get_balance("faction_token")
	set(v): CurrencySystem.set_currency("faction_token", maxi(0, v))
var sweep_tickets: int:
	get: return CurrencySystem.get_balance("sweep_tickets")
	set(v): CurrencySystem.set_currency("sweep_tickets", maxi(0, v))
...
func add_currency(kind: String, amount: int) -> void:
	# 지갑 재화는 CurrencySystem 으로 위임 → 브리지가 GameData.currency_changed 를 재발신한다.
	match kind:
		"gold", "gems", "faction_token", "sweep_tickets":
			if amount >= 0:
				CurrencySystem.add_currency(kind, amount)
			else:
				CurrencySystem.subtract_currency(kind, -amount)
		"stamina":
...
func _ready() -> void:
	...
	load_state()
	_apply_resets()
	# CurrencySystem(단일 원천) 변경을 GameData.currency_changed 로 브리지
	if not CurrencySystem.currency_changed.is_connected(_on_currency_system_changed):
		CurrencySystem.currency_changed.connect(_on_currency_system_changed)
	# 변경 시 자동 저장
...
# 세이브가 없을 때만 지갑 재화를 기본값으로 시드한다 (기존 잔액은 건드리지 않음).
func _seed_default_currencies() -> void:
	for kind in _WALLET_DEFAULTS:
		if CurrencySystem.get_balance(kind) == 0:
			CurrencySystem.set_currency(kind, int(_WALLET_DEFAULTS[kind]))


# CurrencySystem 잔액 변경 → GameData.currency_changed(kind, amount) 로 재발신.
func _on_currency_system_changed(currency_type: String, amount: int, _new_balance: int) -> void:
	currency_changed.emit(currency_type, amount)
...
func save_state() -> void:
	...
	var d := {
		...
		# 재화는 CurrencySystem(단일 원천) 스냅샷으로 저장한다.
		"currencies": CurrencySystem.get_all_currencies(),
...
func load_state() -> void:
	...
	# 재화 복원 → CurrencySystem(단일 원천). currencies 스냅샷 우선, 없으면 구 세이브 호환.
	if d.has("currencies") and typeof(d["currencies"]) == TYPE_DICTIONARY:
		for ct in d["currencies"]:
			CurrencySystem.set_currency(ct, int(d["currencies"][ct]))
	else:
		for legacy in ["gold", "gems", "faction_token", "sweep_tickets"]:
			if d.has(legacy):
				CurrencySystem.set_currency(legacy, int(d[legacy]))
